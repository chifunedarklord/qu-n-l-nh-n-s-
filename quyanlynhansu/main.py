...
def menu_projects():
    ...
    choice = input("  Chọn (a-f): ").strip().lower()
    ...
    try:
        if choice == 'a':
            ...
            result = company.get_top_most_projects(10)
            print(f"\n  Top {len(result)} nhân viên tham gia NHIỀU dự án nhất:")
            print(f"  {'STT':<5} {'Tên':<20} {'Chức vụ':<12} {'Số dự án':<10} {'Tên dự án'}")
            print(f"  {'-'*70}")
            for i, emp in enumerate(result, 1):
                projects_str = ', '.join(emp.projects) if emp.projects else '(chưa có)'
                print(f"  {i:<5} {emp.name:<20} {emp.get_role():<12} {len(emp.projects):<10} {projects_str}")
        elif choice == 'e':
            result = company.get_top_least_projects(10)
            print(f"\n  Top {len(result)} nhân viên tham gia ÍT dự án nhất:")
            print(f"  {'STT':<5} {'Tên':<20} {'Chức vụ':<12} {'Số dự án':<10} {'Tên dự án'}")
            print(f"  {'-'*70}")
            for i, emp in enumerate(result, 1):
                projects_str = ', '.join(emp.projects) if emp.projects else '(chưa có)'
            ...
            project = input("  Tên dự án: ").strip()
            result  = company.get_employees_in_project(project)
            print(f"\n  Thành viên tham gia dự án '{project}' ({len(result)} người):")
            print(f"  {'STT':<5} {'ID':<10} {'Tên':<20} {'Chức vụ':<12} {'Phòng ban'}")
            print(f"  {'-'*65}")
            for i, emp in enumerate(result, 1):
                print(f"  {i:<5} {emp.employee_id:<10} {emp.name:<20} {emp.get_role():<12} {emp.department}")
        else:
            print("  ✗ Lựa chọn không hợp lệ")
    except EmployeeNotFoundError as e:
        print(f"  ⚠ {e}")
...
def menu_hr():
    ...
    print("  d. Cắt giảm nhân sự hàng loạt (nhiều nhân viên nghỉ việc)")
    choice = input("  Chọn (a/b/c/d): ").strip().lower()
    ...
    try:
        if choice == 'a':
            ...
            print("  Nhập các ID nhân viên cần cho nghỉ việc (cách nhau bằng dấu phẩy)")
            print("  Ví dụ: INT001, INT002, DEV003")
            raw = input("  Nhập ID: ").strip()
            if not raw:
                print("  ✗ Chưa nhập ID nào")
            else:
                ids = [x.strip() for x in raw.split(',') if x.strip()]
                print(f"\n  Xác nhận cho {len(ids)} nhân viên nghỉ việc: {', '.join(ids)}")
                confirm = input("  Xác nhận? (y/n): ").strip().lower()
                if confirm == 'y':
                    removed, errors = company.bulk_layoff(ids)
                    if removed:
                        print(f"\n  ✓ Đã cho nghỉ việc {len(removed)} nhân viên:")
                        for emp in removed:
                            print(f"    - {emp.employee_id} | {emp.name} | {emp.get_role()} | {emp.department}")
                    if errors:
                        print(f"\n  ⚠ Không tìm thấy {len(errors)} ID:")
                        for err in errors:
                            print(f"    - {err}")
                else:
